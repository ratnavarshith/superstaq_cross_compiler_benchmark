...
**Status:** Phases 1-4 done. Circuit generators, Qiskit baseline, SuperstaQ cross-compiler harness, and result analysis are all tested.
...
- **Phase 5, Fidelity:** simulate compiled circuits under realistic noise models per backend, compare output fidelity, not just gate count.
- **Phase 6, Larger n:** extend to n in {10, 12} to see if the ring-topology advantage survives or caps out (n=8 ring is already a tie for the Qiskit-only data).
